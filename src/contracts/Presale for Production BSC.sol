...
contract PresaleV4 is  ReentrancyGuard, Ownable, Pausable {
  ...
  event MaxTokensUpdated(uint256 prevValue, uint256 newValue, uint256 timestamp);
      ...
      constructor(address _paymentWallet, address _admin) {
        ...
        startTime = 1686891600;
        endTime = 2008818000;
        rounds[0] = [2500000000,7500000000,15000000000,25000000000,35000000000,45000000000,55000000000,65000000000,75000000000,90000000000];
        ...
        rounds[2] = [1719378000,1751518800,1783659600,1815800400,1848027600,1880168400,1912309200,1944450000,1976677200,2008818000];

        whitelistClaimOnly = false;
        paymentWallet = _paymentWallet; 
        admin = _admin;

        USDTInterface = IERC20(0x55d398326f99059fF775485246999027B3197955); 
        aggregatorInterface = Aggregator(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE); 
    }
